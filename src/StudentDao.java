...
public class StudentDao {
	...
	public static int save(Student s) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("insert into freereport_student(name, email, course, fee, paid, due, address, city, state, country, contacno) values(?,?,?,?,?,?,?,?,?,?,?)");
	...
	public static int update(Student s) {
		...
		try {
			...
			Connection con = AccountantDao.getCon();
			PreparedStatement ps = con.prepareStatement("update freereport_student set name =?, email=?, course=?, fee=?, paind=?, address=?, city=?, state=?, country=?, contactno=? where roolno=?");
			ps.setString(1, s.getName());
			ps.setString(2, s.getEmail());
			ps.setString(3, s.getCourse());
			ps.setInt(4, s.getFee());
	...
	public static List<Student> view(){
		...
		try {
			...
			Connection con=AccountantDao.getCon();
			PreparedStatement ps = con.prepareStatement("select * from freereport_student");
			...
			while(rs.next()) {
				...
				s.setContactno(rs.getString("contacno"));
				list.add(s);
	...
	public static Student getStudentByRollno(int rollno) {
		...
		try {
			...
			Connection con = AccountantDao.getCon();
			PreparedStatement ps= con.prepareStatement("select * form freereport_student where rollno=?");
			ps.setInt(1, rollno);
			ResultSet rs = ps.executeQuery();
			if(rs.next()){
				s.setRollno(rs.getInt("rollno"));
